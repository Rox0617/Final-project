...
public class TQController {
    ...
    public String getTianQiInfo(String city){
        ...
        // The first fetch goes to redis to see if there is any, if there is a direct fetch.
        // 没有的话，然后调用接口，获取到的数据存到redis数据库中
        // If not, then call the interface and get the data stored in the redis database
        if(redisUtil.get("tianqi") != null){
            info = redisUtil.get("tianqi");
        }else{
            //接口地址 Interface address https://www.mxnzp.com/api/weather/current/{城市名}
